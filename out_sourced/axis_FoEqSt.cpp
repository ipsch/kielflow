...
double axis_FoEqSt::k_val_at(const int &index) const
{
	double pi = acos(-1.);
	return ( (index<=N/2) ? (2.*pi*index/L) : (2.*pi*(index-N)/L) );
}
...
int axis_FoEqSt::k_index_at(const double &val) const
{
	std::cout << "axis_FoEqSt::k_index_at(const double &val) const not implemented\n";
	throw("axis_FoEqSt::k_index_at(const double &val) const not implemented\n");
	return 0;
}
